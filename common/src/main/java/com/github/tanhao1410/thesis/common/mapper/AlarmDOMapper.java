...
public interface AlarmDOMapper {
     ...
     * 通用查询行数方法，可设置查询属性
     */
    Long selectCountSelective(@Param("record") AlarmDO record);

    /**
     * 通用查询List方法，可设置查询属性，排序字段，分页参数 
     */
    List<AlarmDO> selectPageSelective(@Param("record") AlarmDO record, @Param("pageable") Pageable pageable);
     ...
     */
    Integer insertSelectiveReturnPrimaryKey(AlarmDO record);

    /**
     * 批量插入行数据返回插入行数
     */
    Integer batchInsert(List<AlarmDO> records);

    /**
     * 根据主键批量获取
     */
    List<AlarmDO> batchSelectByPrimaryKey(List<Long> pkIds);

    /**
     * insertSelective的增强版，插入成功返回1，失败返回0
     * 如果插入的字段中有UNIQUE KEY, KEY已经存在则不予插入返回0，不会抛出DuplicateKeyException
     */
    Integer insertIgnoreSelective(AlarmDO record);
}