...
public interface GroupDOMapper {
     ...
     * 通用查询行数方法，可设置查询属性
     */
    Long selectCountSelective(@Param("record") GroupDO record);

    /**
     * 通用查询List方法，可设置查询属性，排序字段，分页参数 
     */
    List<GroupDO> selectPageSelective(@Param("record") GroupDO record, @Param("pageable") Pageable pageable);
     ...
     */
    Integer insertSelectiveReturnPrimaryKey(GroupDO record);

    /**
     * 批量插入行数据返回插入行数
     */
    Integer batchInsert(List<GroupDO> records);

    /**
     * 根据主键批量获取
     */
    List<GroupDO> batchSelectByPrimaryKey(List<Long> pkIds);

    /**
     * insertSelective的增强版，插入成功返回1，失败返回0
     * 如果插入的字段中有UNIQUE KEY, KEY已经存在则不予插入返回0，不会抛出DuplicateKeyException
     */
    Integer insertIgnoreSelective(GroupDO record);
}